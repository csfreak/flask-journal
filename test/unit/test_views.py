        ...
        with app.test_request_context():
            ...
            for action in [None, 'view', 'new', 'edit']:
                ...
                with self.subTest(action=action):
                    ...
                    self.assertIn(html_test_strings['title'] % f"{action.title()} Journalbasemodel", r)
...
class TableViewFunctionTest(UserAppTestBase):
    ...
    def setUp(self: t.Self, app: Flask) -> None:
        ...
        self.pg_order_query_mock = self.mock_utils.build_query.return_value.order_by.return_value.paginate.return_value
        self.pg_query_mock = self.mock_utils.build_query.return_value.paginate.return_value
    ...
    def test_no_results(self: t.Self, app: Flask) -> None:
        ...
        with app.test_request_context(self.test_table_route):
            ...
            self.assertIn(html_test_strings['table']['create'] % self.test_table_route, rv)
    ...
    def test_1_result(self: t.Self, app: Flask) -> None:
        ...
        with app.test_request_context(self.test_table_route):
            ...
            for item in test_items:
                self.assertIn(html_test_strings['table']['row'] % (item.id, item.id, item.created_at), rv)
            self.assertIn(html_test_strings['table']['pager']['form'], rv)
            self.assertIn(html_test_strings['table']['create'] % self.test_table_route, rv)
    ...
    def test_20_result(self: t.Self, app: Flask) -> None:
        ...
        with app.test_request_context(self.test_table_route):
            ...
            for item in test_items[10:]:
                self.assertNotIn(html_test_strings['table']['row'] % (item.id, item.id, item.created_at), rv)
            self.assertIn(html_test_strings['table']['pager']['form'], rv)
    ...
    def test_order(self: t.Self, app: Flask) -> None:
        ...
        with app.test_request_context(self.test_table_route):
            ...
            self.assertIn(html_test_strings['table']['pager']['form'], rv)
            self.assertIn(html_test_strings['table']['create'] % self.test_table_route, rv)
            self.mock_utils.build_query.return_value.order_by.assert_called_once_with(self.model_class.created_at)
