...
class UserViewClientTest(AppClientTestBase):
    ...
    def test_user_view_post_invalid_role(self: t.Self, app: Flask, client: FlaskClient) -> None:
        ...
        self.assertIn(html_test_strings['title'] % 'View User', rv.text)
        self.assertIn(html_test_strings['form']['id'] % 3, rv.text)
        self.assertIn(html_test_strings['form']['error'] % 'invalid role(s): non-role', rv.text)
...
class EntryViewClientTest(AppClientTestBase):
    ...
    def test_entry_create_view_post_new_tags(self: t.Self, app: Flask, client: FlaskClient) -> None:
        email = 'user3@example.test'
        content = 'lorem ipsum'
        authenticate(client, email)
        rv = client.post('/entry', data={'Title': 'entry 1', 'Body': content, 'Tags': 'testtag', 'Create': 'Create'})
        self.assertStatus(rv, 200)
        self.assertIn(html_test_strings['title'] % 'View Entry', rv.text)
        entry = models.Entry.query.first()
        self.assertIsInstance(entry, models.Entry)
    ...
    def test_entry_create_view_post_existing_tags(self: t.Self, app: Flask, client: FlaskClient) -> None:
        email = 'user3@example.test'
        content = 'lorem ipsum'
...
class UserSettingsViewClientTest(AppClientTestBase):
    def test_settings_view(self: t.Self, app: Flask, client: FlaskClient) -> None:
        ...
        self.assertIn(html_test_strings['settings']['selected'] % ('default', 'default'), rv.text)
        for theme in list(Theme):
            if theme != 'default':
                self.assertIn(html_test_strings['settings']['option'] % (theme, theme), rv.text)
    ...
    def test_settings_post(self: t.Self, app: Flask, client: FlaskClient) -> None:
        ...
        for theme in list(Theme):
            with self.subTest(theme=str(theme)):
                ...
                self.assertIn(html_test_strings['title'] % 'Settings', rv.text)
                self.assertIn(html_test_strings['settings']['selected'] % (theme, theme), rv.text)
                if str(theme) == 'default':
                    self.assertIn(html_test_strings['settings']['css']['default'], rv.text)
                else:
                    self.assertIn(html_test_strings['settings']['css']['bootswatch'] % theme, rv.text)
