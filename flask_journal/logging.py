...
def init_logging(app: Flask) -> None:
    ...
    if app.debug or app.testing:
        ...
        logging_config['loggers']['sqlalchemy.engine.Engine'] = {'level': 'INFO', 'handlers': ['null']}
    else:
...
REQUEST_LOG_RE = re.compile(
    r'(?P<ip>\d{1,3}(\.\d{1,3}){3}) - - \[\d{2}/\w{3}/\d{4} \d{2}:\d{2}:\d{2}\] "%s" %s %s')
...
class Formatter(logging.Formatter):
    ...
    def format(self: t.Self, record: logging.LogRecord) -> str:
        ...
        s: str = self.flask_style.format(record) if record.name.startswith(
            'flask_journal') else self.formatMessage(record)
